...
load('IVCmosPh1.mat');
load('norm_dW1.mat')
load Cyclopean_W1.mat
load Saliency_3d_W1.mat
warning('off')
...
for iPoint=1:330
    ...
    [ img_cropped, S ] = Crop_Saliency_Percentage(Cyclopean_W1(:,:,:,iPoint),Saliency_3d_W1(:,:,iPoint),127,127,Sal_Value,85);        
    %Stock_img{iPoint} = [img_cropped(:,:,:,:)];
    
    Size_val = [Size_val S];
    OUT_Score = repmat(norm_dW1(iPoint),S,1); % prepare norm. dmos for training
    T_output = [T_output;OUT_Score];
    Stock_normdW1{iPoint} = [OUT_Score];
    
    D_Score = repmat(IVCmosPh1(iPoint),S,1); % prepare Dmos for correlation.
...
clear Cyclopean_W1 Saliency_3d_W1
...
for R=1:repeat
    ...
    P = 0;
    for j=1:330        % compute the mean score from patches score
    ...
    disp('Results of Deep-Saliency Waterloo I>>>');
    SB = IVCmosPh1;               % Subjective Score 
