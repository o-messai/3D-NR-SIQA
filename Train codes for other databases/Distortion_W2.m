...
load('IVCmosPh2.mat'); 
load('MOS_W2.mat'); % load database information and DMOS data
...
F = strfind(MOS_W2{iPoint,1},'W');
if ~isempty(F)
SBwn = [SBwn IVCmosPh2(iPoint)];       % Subjective Score
OBwn = [OBwn OB(iPoint)];             % Objective Score
end

%Guassian noise

F = strfind(MOS_W2{iPoint,1},'G');
if ~isempty(F)
SBgb = [SBgb IVCmosPh2(iPoint)];       % Subjective Score
OBgb = [OBgb OB(iPoint)];             % Objective Score
end
%JPEG

F = strfind(MOS_W2{iPoint,1},'J');
if ~isempty(F)
SBjpeg = [SBjpeg IVCmosPh2(iPoint)];       % Subjective Score
