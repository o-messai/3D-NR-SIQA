...
for Sal_Value = [0.3:0.1:0.3]
    ...
    parfor iPoint=1:360
        ...
        [ img_cropped, S ] = Crop_Saliency_Percentage(Cyclopean_L2(:,:,:,iPoint),Saliency_3d_L2(:,:,iPoint),31,31,Sal_Value,80);
...
%         [  trainJPEG_IN,  trainJPEG_OUT,  testJPEG_IN] = Divide_random_kfold(JPEG_crop_input, JPEG_normd_input,k); %clear JPEG_crop_input JPEG_normd_input;
%         [  trainJP2K_IN,  trainJP2K_OUT,  testJP2K_IN] = Divide_random_kfold(JP2K_crop_input, JP2K_normd_input,k); %clear JP2K_crop_input JP2K_normd_input;
%         [  trainWN_IN,    trainWN_OUT,    testWN_IN]   = Divide_random_kfold(WN_crop_input,   WN_normd_input,k);   %clear WN_crop_input WN_normd_input;
%         [  trainBLUR_IN,  trainBLUR_OUT,  testBLUR_IN] = Divide_random_kfold(BLUR_crop_input, BLUR_normd_input,k); %clear BLUR_crop_input BLUR_normd_input;
%         [  trainFF_IN,    trainFF_OUT,    testFF_IN]   = Divide_random_kfold(FF_crop_input,   FF_normd_input,k);   %clear FF_crop_input FF_normd_input;
%         
        %for i=1:k
        ...
        net = []; net_help = alexnet; newLayers = net_help.Layers(2:end-10);
        newLayers(4, 1).Stride = [1 1];
        newLayers(8, 1).Stride = [1 1];
        %newLayers(15, 1).Stride = [1 1];
         layers = [
        ...
        clearvars -except m k Performance Sal_Value Stock_normdL2 Stock_Dmos_new Stock_img Final_R Percentage index FINAL_score Average_R Saliency_vs_Score Models Dmos norm_dL2 Cyclopean_L2 Saliency_3d_L2 X Y
    end
