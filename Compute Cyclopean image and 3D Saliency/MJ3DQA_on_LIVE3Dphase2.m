...
for iPoint = 278 %1:360
    % READ A DISTORTED IMAGE
    
    Id = imread(['/home/temp/Bureau/Oussama/Database/Phase2/Stimuli/' StiFilename{iPoint}]);
    sizeTemp = size (Id);
    %Id = rgb2gray(Id);
    ...
    [CI, dmap_test, GBD_L, GBD_R]  = MJ3DQA(imDL(:,:,1),imDR(:,:,1)); %RED
    Cyclopean_L2(:,:,1,i) = CI;
    %Disparity_L2(:,:,1,i) = dmap_test;
    
    [CI, dmap_test, GBD_L, GBD_R]  = MJ3DQA(imDL(:,:,2),imDR(:,:,2)); %GREEN
    Cyclopean_L2(:,:,2,i) = CI;
    %Disparity_L2(:,:,2,i) = dmap_test;
    
    [CI, dmap_test, GBD_L, GBD_R]  = MJ3DQA(imDL(:,:,3),imDR(:,:,3)); %BLUE
    Cyclopean_L2(:,:,3,i) = CI;
    %Disparity_L2(:,:,3,i) = dmap_test;
...
%     end
     ...
     %depth_map= (300*6.5)./medfilt2(dmap_test, [7 7]);   % Focal lenth 300 pixel- baseline 6.5 cm
    ...
    img = uint8(Cyclopean_L2(:,:,:,i));  % I = 6.3cm, V = 294.64 cm, Rx=75.9cm ,W=129.7cm
...
%     imwrite(norm_smap_3d, './Smap_Results/smap_3d.jpg');
   ...
   Saliency_2d_L2(:,:,i) = norm_smap_2d;
   Saliency_3d_L2(:,:,i) = norm_smap_3d;
