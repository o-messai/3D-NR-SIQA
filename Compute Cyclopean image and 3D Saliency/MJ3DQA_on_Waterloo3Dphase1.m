...
load('IVCmosPh1.mat');
load Filename_W1                                                  % load database information and DMOS data
...
for iPoint = 165:330
      %READ A DISTORTED IMAGE
            ...
            %Id = imread(['/home/temp/Bureau/Oussama/Database/WaterlooIVC3DPhaseI/WaterlooIVC3DPhaseI/3D/Session3DIQ/' Filename{iPoint}]);
            %sizeTemp = size(Id);
            %imDL = Id(:,1:sizeTemp(2)/2,:);
            %imDR = Id(:,sizeTemp(2)/2+1:end,:);  
            
            %imDL = imresize(imDL, [1080   1390]);
            %imDR = imresize(imDR, [1080   1390]);
            %imDL = rgb2gray(imDL);
			%imDR = rgb2gray(imDR);
...
%     end
    ...
    img = uint8(Cyclopean_W1(:,:,:,i));  % I = 6.3cm, V = 294.64 cm, Rx=75.9cm ,W=129.7
    ...
    img = imresize(img, 0.50);
    depth = imresize(depth, 0.50);
...
%    M0 = find(MAP_2D(:)==0); MAP_2D(M0)=0.5;
   ...
   %figure(2), imshow(img), title(num2str(iPoint));
   Saliency_2d_W1(:,:,i) = norm_smap_2d;
   Saliency_3d_W1(:,:,i) = norm_smap_3d;
   disp(iPoint)
    %waitbar(iPoint/330);
    i = i+1;
end
